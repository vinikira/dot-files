...
autocmd('LspAttach', {
    ...
    callback = function(e)
        ...
        end, { buffer = e.buff, desc = "Go to definition" })
        vim.keymap.set("n", "K", function()
            vim.lsp.buf.hover()
        end, { buffer = e.buff, desc = "Show documentation" })
        ...
        vim.keymap.set("n", "<leader>vrn", function()
            vim.lsp.buf.rename()
        end, { buffer = e.buff, desc = "Rename" })
        vim.keymap.set("n", "<leader>vf", function()
            vim.lsp.buf.format()
        end, { buffer = e.buff, desc = "Format (LSP)" })
        vim.keymap.set("i", "<C-h>", function() vim.lsp.buf.signature_help() end,
            { buffer = e.buff, desc = "Signature Help" })
        vim.keymap.set("n", "[d", function()
            vim.diagnostic.goto_next()
        end, { buffer = e.buff, desc = "Next error" })
        vim.keymap.set("n", "]d", function()
            vim.diagnostic.goto_prev()
        end, { buffer = e.buff, desc = "Previous error" })
    end
})
