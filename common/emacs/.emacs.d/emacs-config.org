...
#+begin_src emacs-lisp
  (use-package transform-string-at-point
    :ensure (transform-string-at-point
             :repo "git@github.com:waymondo/transform-string-at-point.git")
    :defer t)
#+end_src
*** Treesit Auto
    ...
    (defun vs/elixir-choose-lsp (server)
      ...
      (with-eval-after-load 'eglot
        (setf (alist-get '(elixir-mode elixir-ts-mode heex-ts-mode) eglot-server-programs nil nil #'equal)
              (pcase server
                ('(?E "ElixirLS")
                 `(,(concat user-emacs-directory "cache/lsp/elixir-ts-mode/elixir-ls/language_server.sh")))
                ('(?L "LexicalLSP")
                 `(,(concat user-emacs-directory "cache/lsp/elixir-ts-mode/lexical/bin/start_lexical.sh")))
                ('(?N "NextLS")
                 `(,(concat user-emacs-directory "/cache/lsp/elixir-ts-mode/next-ls/next_ls")
                   "--stdio=true"
