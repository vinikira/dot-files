...
#+begin_src emacs-lisp
  (use-package project
    ...
    :config
    (fset 'project-shell 'ghostel-project))
#+end_src
...
Repo: https://github.com/LuciusChen/clutch
...
#+begin_src emacs-lisp
  (use-package transform-symbol-at-point
    :vc (:url "git@github.com:waymondo/transform-symbol-at-point.git" :rev :newest)
...
#+begin_src emacs-lisp
  ...
  Uses position instead of index field."
    ...
    ("H-P" . vs/tab-group-from-project)
    ("C-x t g" . vs/tab-switch-to-group)
    ("H-K" . vs/close-project-tab)
...
#+begin_src emacs-lisp
  ...
  (keymap-global-set "C-x 1" 'vs/toggle-delete-other-windows)
  ...
  (keymap-global-set "H-f" 'vs/find-org-file)
...
#+begin_src emacs-lisp
  (use-package nxml-mode
    ...
    :bind (:map nxml-mode-map
                ("C-c C-f" . xml-format-buffer)
                ("C-c C-w" . vs/nxml-where))
    :config
    (reformatter-define xml-format
      :program "xmllint"
      :args '("--nowarning" "--format" "-")))
#+end_src
...
#+begin_src emacs-lisp
  (use-package elixir-ts-mode
    ...
    (defun vs/elixir-choose-lsp (server)
      ...
      (with-eval-after-load 'eglot
        (setf (alist-get '(elixir-mode elixir-ts-mode heex-ts-mode) eglot-server-programs nil nil #'equal)
              (pcase server
                ('(?X "ExpertLS")
                 `(,(expand-file-name (concat user-emacs-directory "cache/lsp/elixir-ts-mode/expert/expert")) "--stdio"))
                ('(?E "ElixirLS")
    ...
    (vs/elixir-choose-lsp '(?X "ExpertLS")))
#+end_src
...
#+begin_src emacs-lisp
  (with-eval-after-load 'elixir-ts-mode
    ...
    (tempo-define-template "elixir-mode-pry"
                           ...
                           "IEx pry"
                           'elixir-tempo-tags)
    (tempo-define-template "elixir-ecto-schema"
                           '(
                             (let ((fields '())
                                   (field-name "")
                                   (field-type "")
                                   (is-required ""))
                               (while (not (string= field-name "stop"))
                                 (setq field-name (read-string "Enter field name (or type 'stop' to finish): "))
                                 (unless (string= field-name "stop")
                                   (setq field-type (read-string (format "Enter type for field '%s': " field-name)))
                                   (setq is-required (y-or-n-p (format "Is field '%s' required? " field-name)))
                                   (push (list field-name field-type is-required) fields)))
                               `(l
                                 "use Ecto.Schema"
                                 n>
                                 ...
                                 ,(mapconcat (lambda (field)
                                               (format "%s: %s,"
                                                       (nth 0 field)   ; Field name
                                                       (pcase (nth 1 field) ; Field type
                                                         (":string" "String.t()")
                                                         (":id" "integer()")
                                                         (":binary_id" "binary()")
                                                         (":binary" "binary()")
...
#+begin_src emacs-lisp :tangle no
  (use-package compile-eslint
    :vc (:url "https://github.com/Fuco1/compile-eslint.git" :rev :newest)
...
#+begin_src emacs-lisp
  (use-package emmet-mode
    ...
    :hook
    ((web-mode vue-mode js-mode js-ts-mode heex-ts-mode elixir-ts-mode nxml-mode)
     . emmet-mode))
#+end_src
